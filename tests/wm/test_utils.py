...
def test_save_pretrained_saves_weights_file(tmp_path):
    model = TinyModel()
    save_pretrained(model, run_name="run1", config=TINY_OMEGACONF, cache_dir=tmp_path)
...
def test_save_pretrained_saves_config_json(tmp_path):
    model = TinyModel()
    save_pretrained(model, run_name="run1", config=TINY_OMEGACONF, cache_dir=tmp_path)
    saved = json.loads((_ckpt_root(tmp_path) / "run1" / "config.json").read_text())
...
def test_save_pretrained_state_dict_matches(tmp_path):
    model = TinyModel()
    save_pretrained(model, run_name="run1", config=TINY_OMEGACONF, cache_dir=tmp_path)
    loaded = torch.load(_ckpt_root(tmp_path) / "run1" / "weights.pt", map_location="cpu")
...
def test_save_pretrained_custom_filename(tmp_path):
    model = TinyModel()
    save_pretrained(model, run_name="run1", config=TINY_OMEGACONF, filename="epoch_5.pt", cache_dir=tmp_path)
...
def test_save_pretrained_config_key_extracts_subconfig(tmp_path):
    ...
    save_pretrained(model, run_name="run1", config=cfg, config_key="model", cache_dir=tmp_path)
    saved = json.loads((_ckpt_root(tmp_path) / "run1" / "config.json").read_text())
...
def test_save_pretrained_creates_run_directory(tmp_path):
    model = TinyModel()
    save_pretrained(model, run_name="new/nested/run", config=TINY_OMEGACONF, cache_dir=tmp_path)
    assert (_ckpt_root(tmp_path) / "new" / "nested" / "run" / "weights.pt").exists()
...
def test_load_pretrained_weights_match(tmp_path):
    ...
    for key in original.state_dict():
        torch.testing.assert_close(original.state_dict()[key], loaded.state_dict()[key])
...
def test_roundtrip_preserves_weights(tmp_path):
    original = TinyModel()
    save_pretrained(original, run_name="rt_run", config=TINY_OMEGACONF, cache_dir=tmp_path)
    with patch("hydra.utils.instantiate", return_value=TinyModel()):
...
def test_roundtrip_custom_filename(tmp_path):
    original = TinyModel()
    save_pretrained(original, run_name="rt_run", config=TINY_OMEGACONF, filename="epoch_3.pt", cache_dir=tmp_path)
    with patch("hydra.utils.instantiate", return_value=TinyModel()):
