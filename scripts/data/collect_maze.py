...
def _resolve_maps(cfg) -> tuple[dict | None, Path | None]:
    ...
    if exclude_path is not None:
        ...
        if isinstance(raw, dict):
            exclude_maps = (
                {v: True for v in raw.values()}
                if all(isinstance(v, str) for v in raw.values())
                else raw
            )
        logging.info(
            f'Loaded {len(exclude_maps)} maps to exclude from {exclude_path}'
        )
    ...
    if maps is not None:
        ...
        if start_idx is not None and end_idx is not None:
            maps = {
                k: v for k, v in maps.items() if start_idx <= int(k) <= end_idx
            }
            logging.info(
                f'Filtered to {len(maps)} maps (idx {start_idx}–{end_idx})'
            )
    ...
    if (
        maps is not None
        and train_maps_n is not None
        and len(maps) > train_maps_n
    ):
...
def _collect_with_variation(cfg, maps_file, maps):
    ...
    options = (
        OmegaConf.to_object(cfg.get('options')) if cfg.get('options') else None
    )
...
def run(cfg):
    ...
    if cfg.get('gen_map_only', False):
        logging.success(
            'gen_map_only=true — maps generated, skipping data collection.'
        )
        return
